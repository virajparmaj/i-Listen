...
import { GENRES, OUTPUT_OPTIONS, PRESETS, presetFor } from "../data/mockData.js";
...
export function MetadataEditor({ open, track, onClose, onSave, resizeArtwork = true }) {
  const [draft, setDraft] = React.useState(track);
  React.useEffect(() => setDraft(track), [track]);
  if (!track || !draft) return null;

  const set = (key) => (e) => setDraft({ ...draft, [key]: e.target.value });
  const setPlaylists = (e) => {
    const playlists = e.target.value.split(/[,\n]+/).map((item) => item.trim()).filter(Boolean);
    setDraft({ ...draft, playlistText: e.target.value, playlists });
  };
  const setPreset = (e) => {
    const nextPreset = presetFor(e.target.value);
    setDraft({ ...draft, preset: nextPreset.id, outputOption: nextPreset.outputOption });
  };
  const genreOpts = [{ value: "", label: "Unspecified" }, ...GENRES.map((g) => ({ value: g, label: g }))];
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: 20 }}>
        <div>
          <CoverArtUploader value={draft.coverArt} onChange={(d) => setDraft({ ...draft, coverArt: d })} compact defaultResize={resizeArtwork} />
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
            <Field label="Output">
              <Select
                value={draft.outputOption}
                onChange={set("outputOption")}
                options={OUTPUT_OPTIONS.map((o) => ({ value: o.value, label: o.label }))}
                style={{ width: "100%" }}
              />
            </Field>
            <Field label="Quality preset">
              <Select value={draft.preset} onChange={setPreset} options={presetOptions} style={{ width: "100%" }} />
            </Field>
          </div>
        </div>
